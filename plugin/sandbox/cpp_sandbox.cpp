	...
	void setup_policy() {
		...
		static auto const open_rule = [](sandbox_t const * sandbox, event_t const * event, action_t * action) {
			static auto const valid_arguments = {
				...
				std::make_pair("/lib/x86_64-linux-gnu/libgcc_s.so.1",      (O_RDONLY | O_CLOEXEC)),
				std::make_pair("/lib/x86_64-linux-gnu/libm.so.6",          (O_RDONLY | O_CLOEXEC)),
				std::make_pair("/usr/lib/x86_64-linux-gnu/libstdc++.so.6", (O_RDONLY | O_CLOEXEC)),
			};
			auto addr = event->data._SYSCALL.a;
